...
///   **To reconnect, build a new client.**
...
public protocol NetworkClient: AnyObject, Sendable {
    ...
    /// Connection lifecycle, transmit transitions, watchdog expiry, DTMF,
    /// remote station activity and audio problems, in order.
    ///
    ...
    ///
    /// Buffered without limit — an event dropped because the UI was briefly
    /// busy is an event the operator never sees, and SF-1 says they must see the
    /// watchdog fire. Finished by ``disconnect()``.
    ///
    ...
    ///   length the mode's codec frames at (160 samples — 20 ms — for G.711
    ///   µ-law, Codec2 3200 and GSM 06.10 alike).
