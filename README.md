...
| `M17Kit` | Complete. Reflector control, base-40 callsigns, stream mode both ways, Codec 2 3200 (FR-2.4) via two interchangeable implementations — `WeebillVoiceCodec` (pure Swift, the default) and `Codec2VoiceCodec` (the LGPL XCFramework, opt-in) — and `M17Client`. Both directions confirmed against live reflectors — see below. Weebill was proven both ways on 2026-08-28 — receiving a net, and transmitting to a second client running the reference codec2 framework — but not yet against a third-party station. |
