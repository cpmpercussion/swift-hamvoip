...
final class NetworkClientTests: XCTestCase {
    ...
    /// Everything SF-1…SF-4 and the app need, all representable.
    ...
    /// Remote transmit activity is a different axis from ``RadioEvent/receiving``
    /// — that one is about *our* PTT — so the two must not collapse into each
    /// other, and "was cut off" must not read the same as "stopped".
    func testRemoteTransmitActivityIsDistinctFromOurOwnTransmitState() {
        XCTAssertNotEqual(RadioEvent.remoteTransmitStarted(), .transmitting)
        XCTAssertNotEqual(RadioEvent.remoteTransmitEnded(), .receiving)
        XCTAssertNotEqual(
            RadioEvent.remoteTransmitEnded(station: "VK1XYZ", displaced: true),
            .remoteTransmitEnded(station: "VK1XYZ", displaced: false))

        // Being talked over is the case an operator needs to see named.
        let displaced = "\(RadioEvent.remoteTransmitEnded(station: "VK1XYZ", displaced: true))"
        XCTAssertTrue(displaced.contains("VK1XYZ"))
        XCTAssertTrue(displaced.contains("cut off"))
        XCTAssertFalse("\(RadioEvent.remoteTransmitEnded(station: "VK1XYZ"))".contains("cut off"))

        // A mode with no per-over identity still renders as something sayable.
        XCTAssertFalse("\(RadioEvent.remoteTransmitStarted())".isEmpty)
    }
