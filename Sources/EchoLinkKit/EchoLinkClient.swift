...
public enum EchoLinkClientError: Error, Equatable, CustomStringConvertible {
    ...
    /// ``EchoLinkClient/stopTransmit()`` — or another, concurrent
    /// ``EchoLinkClient/startTransmit()`` — ran while this call was suspended
...
public actor EchoLinkClient: NetworkClient {
    ...
    /// `IAX2Client.reentrancyTestHook` is, for the same reason.
    ...
    public func connect(
        ...
        // suspended (the proxy stream can close during the directory login,
        // for instance), and reporting a session that is already gone would
        // be a lie.
        guard phase == .connecting else { throw EchoLinkClientError.notConnected }
    ...
    /// deliberately does not re-arm: a caller re-keying per frame would push
    /// the deadline out forever, which defeats the requirement.
    ///
    ...
    /// way throws ``EchoLinkClientError/transmitCancelled`` instead of
    /// returning normally, because a silent return here is indistinguishable
    ...
    public func startTransmit() async throws {
        ...
        guard generation == transmitGeneration else {
            await watchdog.cancel(ifCurrent: armed)
            throw EchoLinkClientError.transmitCancelled
        }
    ...
    /// run first. That bump, and the unconditional `watchdog.cancel()` right
    /// after it, both happen strictly before the `state` check below, so the
    /// part-filled-packet flush that check guards is untouched by this: it
    /// still runs exactly where it always did, after the watchdog is down.
